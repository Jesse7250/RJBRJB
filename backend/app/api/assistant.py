...
SYSTEM_PROMPT = """你是「智慧伴学 EduMate」的虚拟数字人导学助手，名字叫“小慧”。你的定位不是替代学习对话里的 AI 导师，而是在课程广场和课程工作台中帮助用户理解页面、按钮、数据含义和下一步操作。
...
class AskRequest(BaseModel):
    ...
    role_context: str | None = Field(default=None, description="当前角色上下文")
    page_context: str | None = Field(default=None, description="当前页面上下文")
    selected_concept: str | None = Field(default=None, description="当前关注主题")
...
async def ask_assistant(payload: AskRequest):
    ...
    if payload.role_context:
        context_hint.append(f"当前角色：{payload.role_context}")
    if payload.page_context:
        context_hint.append(f"当前页面：{payload.page_context}")
    if payload.selected_concept:
        context_hint.append(f"当前关注主题：{payload.selected_concept}")
    messages = [
        {"role": "system", "content": SYSTEM_PROMPT},
        *(
            [{"role": "system", "content": "；".join(context_hint)}]
            if context_hint
            else []
        ),
